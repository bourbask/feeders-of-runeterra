...
| Brulure du souffle | **Deux temps** — la regle veut qu'on voie les des avant de decider : `roll.action_resolved { burnWindow: true }` → intention `momentum.burn` → `character.momentum_burned` + `roll.action_revised` → `move.resolved`. Le premier jet n'est jamais reecrit : on ajoute sa revision |
